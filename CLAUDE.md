...
One persistent state (Hand) + a Tab accessory overlay: **LMB** scoop snow into your hands — on a sculpture it bites out the chunk under the red cursor sphere, on bare ground a handful (mass continuity); while carrying, a tap lets go where the snow already is (fuses into snow, otherwise falls under gravity) and holding charges a throw · **Shift+LMB** smooth/pat (works with a ball in hand) · RMB pick-up is disabled for now · a carried ball rolls at your feet only while the cursor points near you, otherwise it is held overhead · **scroll** brush radius · **Tab** accessory overlay (scroll picks · LMB place · RMB remove). First person: WASD move · mouse look · Space jump · Q crouch · E tiptoe · Esc unlocks cursor (LMB re-locks). Snowfall slowly refills paths (`SnowfallCycle`; hold **N** to let it snow hard). Sculptures auto-save to `persistentDataPath/sculptures/field<N>` on quit and auto-load on start (**F5** save · **F9** reload; `SaveLoadManager`). **Left/Right arrows** switch fields (`FieldSwitcher`): sculptures *and* the trampled heightmap are saved per field (`terrain.bin` beside the sculpture JSONs) and restored on arrival, so nothing carries over; the HUD shows which field you are on. Snowballs are already small sculptures: brush them, attach balls anywhere on their surface, decorate them; attaching fixes them in place and promotes them to a full grid (`SculptureFactory.Fuse/Promote`). Sculpt mode also raises/carves the ground (draw in the snow); walking and rolling press paths into it.
