...
One persistent state (Hand) + a Tab accessory overlay: **LMB** smooth/pat snow · **Shift+LMB** scoop: bite a chunk out of a sculpture and hold that exact chunk (mass continuity; the red cursor sphere is the bite) · **RMB** pick up the aimed ball, sculpture or accessory; while carrying, a tap lets go where the snow already is (fuses into snow, otherwise falls under gravity) and holding charges a throw; on bare ground RMB scoops a handful · a carried ball rolls at your feet only while the cursor points near you, otherwise it is held overhead · **scroll** brush radius · **Tab** accessory overlay (scroll picks · LMB place · RMB remove). First person: WASD move · mouse look · Space jump · Q crouch · E tiptoe · Esc unlocks cursor (LMB re-locks). Snowfall slowly refills paths (`SnowfallCycle`; hold **N** to let it snow hard). Sculptures auto-save to `persistentDataPath/sculptures` on quit and auto-load on start (**F5** save · **F9** reload; `SaveLoadManager`). Snowballs are already small sculptures: brush them, attach balls anywhere on their surface, decorate them; attaching fixes them in place and promotes them to a full grid (`SculptureFactory.Fuse/Promote`). Sculpt mode also raises/carves the ground (draw in the snow); walking and rolling press paths into it.
